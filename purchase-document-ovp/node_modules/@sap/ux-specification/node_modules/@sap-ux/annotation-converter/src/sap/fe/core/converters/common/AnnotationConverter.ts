...
export function convertTypes(parserOutput: ParserOutput): ConverterOutput {
	...
	toResolve.forEach(resolveable => {
		...
		const annotationsTerm = resolveable.annotationsTerm;
		const annotationType = resolveable.annotationType;
		...
		if (!resolveable.$target) {
			...
			if (annotationsTerm && annotationType) {
				...
				const property = resolveable.term;
				const path = resolveable.path;
