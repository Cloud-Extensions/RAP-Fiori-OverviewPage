    ...
    switch (propertyValue.type) {
        ...
        case "Path":
            if (isAnnotationPath(propertyValue.Path)) {
                ...
                var $target_1 = resolveTarget(objectMap, currentTarget, propertyValue.Path, false, false, annotationType, annotationsTerm);
                if ($target_1) {
                    return $target_1;
                }
            }
        ...
        case "Path":
            return collectionDefinition.map(function (pathValue) {
                if (isAnnotationPath(pathValue.Path)) {
                    // If it's an anntoation that we can resolve, resolve it !
                    var $target_2 = resolveTarget(objectMap, currentTarget, pathValue.Path, false, false, annotationType, annotationsTerm);
                    if ($target_2) {
                        return $target_2;
                    }
                }
    ...
    toResolve.forEach(function (resolveable) {
        ...
        if (!resolveable.$target) {
            ...
            if (annotationsTerm && annotationType) {
                ...
                var property_1 = resolveable.term;
