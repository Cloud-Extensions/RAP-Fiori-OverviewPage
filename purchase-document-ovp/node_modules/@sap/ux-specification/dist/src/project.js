...
const getAnnotations = async (manifest, webAppPath) => {
    ...
    if (serviceDataSources &&
        ...
        for (const annotationRef of serviceDataSources.mainService.settings.annotations) {
            const annotationPath = path_1.join(webAppPath, serviceDataSources[annotationRef].settings.localUri);
            annotationData.push({
                dataSourceUri: annotationPath,
                fileContent: await readFile(annotationPath)
            });
        }
    }
    ...
    if (annotationData.length === 0) {
        for (const key in serviceDataSources) {
            ...
            if (dataSource.type === 'ODataAnnotation' && dataSource.settings && dataSource.settings.localUri) {
                const annotationPath = path_1.join(webAppPath, dataSource.settings.localUri);
                annotationData.push({
                    dataSourceUri: annotationPath,
                    fileContent: await readFile(annotationPath)
                });
