...
function fillSections(sectionProperties, manifestSettings, configPart, manifest, jsonSchema, v4Page) {
    for (const sectionId in sectionProperties) {
        ...
        if (schemaSection['$ref'] && schemaSection['$ref'].startsWith('#/definitions/ObjectPageSectionTableV4')) {
            ...
            if (manifestSection && manifestSection['tableSettings']) {
                ...
                const sectionDefinition = 'ObjectPageTableV4<' + sectionId + '>';
                ...
                if (manifestSection['columns']) {
                    ...
                    for (const columnKey in manifestSection['columns']) {
                        ...
                        if (!column.template) {
                            configPart[sectionId].table.columns[columnKey] = new controls_1.ObjectPageTableColumn();
                            index_1.transferSettingsOfObject(configPart[sectionId].table.columns[columnKey], manifest, jsonSchema['definitions']['TableColumn'], v4Page.id, [columnKey], manifestSectionId);
                        }
